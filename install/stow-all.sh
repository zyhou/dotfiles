...
rm -rf ~/.config/alacritty ~/.config/bat ~/.config/Code/User/settings.json ~/.gitconfig ~/.config/fastfetch/config.conf
rm -rf ~/.config/nvim ~/.config/starship.toml ~/.config/fsh ~/.tmux.conf ~/.config/lazygit/config.yml

echo '[*] Stowing/Creating simlinks'
cd .. && stow -vSt ~ .fonts alacritty bat code git fastfetch nvim starship tmux zsh fsh lazygit
